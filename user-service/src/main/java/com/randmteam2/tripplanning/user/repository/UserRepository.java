...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    // S1-F9: only queries users table — completed-trip count now comes from Feign → itinerary-service
    @Query(value = """
        SELECT * FROM users
        WHERE preferences ->> 'travelStyle' = :style
    """, nativeQuery = true)
    List<User> findByTravelStyle(@Param("style") String style);
}
